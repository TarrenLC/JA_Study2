...
## Split data by emotion regulators
```{r}

emodf <- select(ja_df2, ID, AGEn, PRE_CONFIDENCEn, EMO_REG_EFFORT1n,
                EMO_REG_EFFORT2n, POST_CONFIDENCEn, PERCEIVED_ADVICE_ACCURACYn,
                DASS, JAR, WOA, GMC_AGE, GMC_DASS,EMO_REGULATOR)

emodf <- filter(emodf, EMO_REGULATOR=="YES")

# Rename emotion regulation effort 1 to effort in change
emodf$EMO_CHNG_EFFORT <- emodf$EMO_REG_EFFORT1n

# Rename emotion regulation effort 2 to effort in acceptance
emodf$EMO_ACPT_EFFORT <- emodf$EMO_REG_EFFORT2n
...
emodf <- emodf %>% mutate(EMO_CHNG_EFFORT_GMC  = scale(EMO_CHNG_EFFORT, center = TRUE, scale = FALSE))

emodf <- emodf %>% mutate(EMO_ACPT_EFFORT_GMC  = scale(EMO_ACPT_EFFORT, center = TRUE, scale = FALSE))

# Pivot longer for jar and woa

emodf <- pivot_longer(emodf, cols = c(JAR01_WOA:JAR12_WOA), names_to = "JAR", values_to = "WOA")

# Pivot longer for emotion effort by type

emodf <- pivot_longer(emodf, cols = c(EMO_CHNG_EFFORT_GMC, EMO_ACPT_EFFORT_GMC), 
                      names_to = "EMO_REG_TYPE", values_to = "EMO_EFFORT")
