...
## LMM with Emotion Regulation types as predictor
```{r}


model27 <- lmer(WOA~GMC_AGE + GMC_DASS + EMO_REG_TYPE +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

summary(model27)

model28 <- lmer(WOA~GMC_AGE*EMO_REG_TYPE + GMC_DASS +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
...
model29 <- lmer(WOA~GMC_AGE + EMO_REG_TYPE*GMC_DASS +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model27, model29)

summary(model29) # Retain model.

# Add pre-confidence

model30 <- lmer(WOA~GMC_AGE + EMO_REG_TYPE*GMC_DASS + GMC_PRE_CONF +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model30)

model31 <- lmer(WOA~GMC_AGE + EMO_REG_TYPE*GMC_DASS+ EMO_REG_TYPE*GMC_PRE_CONF +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model31)

model32 <- lmer(WOA~GMC_AGE + EMO_REG_TYPE*GMC_DASS + GMC_PRE_CONF + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model32)

summary(model32) # Retain model 32

model33 <- lmer(WOA~GMC_AGE + EMO_REG_TYPE*GMC_DASS + GMC_PRE_CONF + EMO_REG_TYPE*GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
...
#2.1 Linearity of relationships 
linearity_model32 <- plot_model(model32, type = "resid",  show.data = TRUE) 
linearity_model32
#2.3 Normality of residuals
#FOR BOTH 2.2 and 2.3, we're going to use this command from sjPlot (as well as for some of our "other checks" in 2.5)
#sjPlot has a suite of other diagnostic plots for each model - let's run these plots (type = "diag") and save them to a new object
#you'll get a list of plots in one object from this command - we'll pull them in step-by-step
model32_diagnostics1 <- visualize(model32, plot = "residuals")
...
#### Robust LMM
```{r}
model32_robust <- rlmer(WOA~GMC_AGE + EMO_REG_TYPE*GMC_DASS + GMC_PRE_CONF + GMC_PERCEIVED_ACC + 
                          (1|ID), data = ja_df4, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

summary(model32_robust)
...
tab_model(model32_robust)

plot_model(model32_robust, type = "resid",  show.data = TRUE, show.loess = FALSE) 
...
tab_model(model32_robust,
          ...
          dv.labels = c("Weight of Advice"), #now I'm saying what I want my outcome variable to be labelled
          file = "model32_Robust.html")


## Visualisation of model
plot_model(model32_robust)
```
...
mod32_emm <- emtrends(model32_robust, var = "GMC_DASS", 
...
plot_model32_int <- plot_model(model32_robust, type = "pred", #can use the same model type as before from sjplot
...
### Model with fluid IQ added
```{r}

model34 <- lmer(WOA~GMC_AGE + EMO_REG_TYPE*GMC_DASS + GMC_PRE_CONF + GMC_PERCEIVED_ACC + GMC_FLUIDIQ +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
