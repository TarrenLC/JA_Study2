...
model6 <- lmer(WOA~GMC_DASS + EMO_REGULATOR + (1|ID), data = ja_df2, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
summary(model6)

model7 <- lmer(WOA~GMC_DASS*EMO_REGULATOR + (1|ID), data = ja_df2, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
summary(model7)
...
model9 <- lmer(WOA~GMC_AGE + GMC_DASS + (1|ID), data = ja_df2, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
summary(model9) # Without emotion regulation
...
ja_df2 <- spread(ja_df2, JAR, WOA)

# Create Grand Mean Centered 

ja_df2 <- ja_df2 %>% mutate(EMO_CHNG_EFFORT_GMC  = scale(EMO_CHNG_EFFORT, center = TRUE, scale = FALSE))

ja_df2 <- ja_df2 %>% mutate(EMO_ACPT_EFFORT_GMC  = scale(EMO_ACPT_EFFORT, center = TRUE, scale = FALSE))
...
cor_df <- select(ja_df3, AGEn, AGEGROUP, DASS, EMO_CHNG_EFFORT, EMO_ACPT_EFFORT, 
                 AVFLUIDIQ, AVG_WOA)

# For only older adults
cor_df_older <- cor_df %>% filter(AGEGROUP == "OLDER")

cor_df_older <- select(cor_df_older, AGEn, DASS, EMO_CHNG_EFFORT, EMO_ACPT_EFFORT,
                       AVFLUIDIQ, AVG_WOA)

# For all
cor_df <- select(ja_df3, AGEn, DASS, EMO_CHNG_EFFORT, EMO_ACPT_EFFORT, 
                 AVFLUIDIQ, AVG_WOA)

```


#### Correlation matrix Older Adults
```{r}

rcorr(as.matrix(cor_df_older), type = "spearman")
...
### Split data by the emotion regulators
```{r}
...
model10 <- lmer(WOA~GMC_AGE + GMC_DASS + EMO_EFFORT +
                  (1|ID), data = emodf, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

summary(model10)

model11 <- lmer(WOA~GMC_AGE + GMC_DASS + EMO_EFFORT + REGULATION_TYPE +
                  (1|ID), data = emodf, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
...
model12 <- lmer(WOA~GMC_AGE + GMC_DASS + GMC_AGE*EMO_EFFORT +
                  (1|ID), data = emodf, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

summary(model12)

model13 <- lmer(WOA~GMC_AGE + GMC_DASS*EMO_EFFORT +
                  (1|ID), data = emodf, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
...
### Correlation matrix among only emotion regulators
```{r}

emodf <- spread(emodf, JAR, WOA)

emodf <- spread(emodf, REGULATION_TYPE, EMO_EFFORT)

emodf_cor <- select(emodf, AGEn, DASS, AVFLUIDIQ, EMO_CHNG_EFFORT, 
                    EMO_ACPT_EFFORT, AVG_WOA)


# Combine effort into one column

emodf_cor <- emodf_cor %>% 
  mutate(EMO_EFFORT = ifelse(is.na(EMO_CHNG_EFFORT), EMO_ACPT_EFFORT,
                             ifelse(is.na(EMO_ACPT_EFFORT), EMO_CHNG_EFFORT)))


rcorr(as.matrix(emodf_cor), type = "spearman")
