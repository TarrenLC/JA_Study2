...
ja_df3$EMO_REGULATOR <- gsub("NO", "0", ja_df3$EMO_REGULATOR)

ja_df3$EMO_REGULATOR <- gsub("YES", "1", ja_df3$EMO_REGULATOR)

ja_df3$EMO_REGULATOR <- as.numeric(ja_df3$EMO_REGULATOR)

structure(ja_df3)

# Select variables for correlation

cor_df <- select(ja_df3, AGEn, DASS, PRE_CONFIDENCEn, POST_CONFIDENCEn,
...
ja_df3$EMO_REGULATOR <- gsub("0", "NO", ja_df3$EMO_REGULATOR)

ja_df3$EMO_REGULATOR <- gsub("1", "YES", ja_df3$EMO_REGULATOR)

ja_df3$EMO_REGULATOR <- as.factor(ja_df3$EMO_REGULATOR)
...
### THIS CODE IS INCORRECT! levels(ja_df4$EMO_REG_TYPE) <- c("NONE", "ACCEPT", "CHANGE")

levels(ja_df4$EMO_REG_TYPE)

ja_df4$EMO_REG_TYPE <- relevel(ja_df4$EMO_REG_TYPE, "NONE")
...
model27 <- lmer(WOA~GMC_AGE + GMC_DASS + EMO_REG_TYPE + GMC_FLUIDIQ + GMC_PRE_CONF + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

summary(model27)

model28 <- lmer(WOA~GMC_AGE*GMC_DASS + EMO_REG_TYPE + GMC_FLUIDIQ + GMC_PRE_CONF + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
...
model29 <- lmer(WOA~GMC_AGE + GMC_DASS*EMO_REG_TYPE + GMC_FLUIDIQ + GMC_PRE_CONF + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model27, model29)

summary(model29) # Retain model 29

tab_model(model29)

model30 <- lmer(WOA~GMC_AGE + GMC_DASS*EMO_REG_TYPE*GMC_FLUIDIQ + GMC_PRE_CONF + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model30)

summary(model30) # Retain model29

model31 <- lmer(WOA~GMC_AGE + GMC_DASS*EMO_REG_TYPE*GMC_PRE_CONF + GMC_FLUIDIQ + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model31) # Retain model 29


model32 <- lmer(WOA~GMC_AGE + GMC_DASS*EMO_REG_TYPE + GMC_FLUIDIQ + GMC_PRE_CONF*GMC_DASS + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model32) # Retain model 29

model33 <- lmer(WOA~GMC_AGE*GMC_DASS*EMO_REG_TYPE + GMC_FLUIDIQ + GMC_PRE_CONF + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model33)

summary(model33) # Retain model 29

model34 <- lmer(WOA~GMC_AGE +GMC_DASS*EMO_REG_TYPE*GMC_PERCEIVED_ACC + GMC_FLUIDIQ + GMC_PRE_CONF +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model34)
...
model35 <- lmer(WOA~GMC_AGE + GMC_DASS*EMO_REG_TYPE + GMC_FLUIDIQ + GMC_PRE_CONF*GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model35)

summary(model35) # Retain model29

model36 <- lmer(WOA~GMC_AGE*GMC_PRE_CONF + GMC_DASS*EMO_REG_TYPE + GMC_FLUIDIQ + GMC_PERCEIVED_ACC +
                  (1|ID), data = ja_df4,  control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

anova(model29, model36)
...
#2.1 Linearity of relationships 
linearity_model29 <- plot_model(model29, type = "resid",  show.data = TRUE) 
linearity_model29
#2.3 Normality of residuals
#FOR BOTH 2.2 and 2.3, we're going to use this command from sjPlot (as well as for some of our "other checks" in 2.5)
#sjPlot has a suite of other diagnostic plots for each model - let's run these plots (type = "diag") and save them to a new object
#you'll get a list of plots in one object from this command - we'll pull them in step-by-step
model29_diagnostics1 <- visualize(model29, plot = "residuals")
model29_diagnostics1

tab_model(model29)

model29_diagnostics2 <- modelDiagnostics(model29, ev.perc = .001)

plot(model29_diagnostics2, ask = FALSE, ncol = 2, nrow = 3)

```

#### Robust LMM
```{r}
model29_robust <- rlmer(WOA~GMC_AGE + GMC_DASS*EMO_REG_TYPE + GMC_FLUIDIQ + GMC_PRE_CONF + GMC_PERCEIVED_ACC +
                          (1|ID), data = ja_df4, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

summary(model29_robust)
...
tab_model(model29_robust)

plot_model(model29_robust, type = "resid",  show.data = TRUE, show.loess = FALSE) 
...
tab_model(model29_robust,
          ...
          dv.labels = c("Weight of Advice"), #now I'm saying what I want my outcome variable to be labelled
          file = "model29_Robust.html")


## Visualisation of model
plot_model(model29_robust)
...
mod29_emm <- emtrends(model29_robust, var = "GMC_DASS", 
...
plot_model29_int <- plot_model(model29_robust, type = "pred", #can use the same model type as before from sjplot
                                ...
                                title = " ", 
                                axis.title = c("Depressive Symptom Score (grand-mean centered)", "Weight of Advice"), 
                                legend.title = "Emotion Regulation Method", #adding a title to the legend
                                show.data = TRUE, dot.size = 0.8, jitter = 0.2, 
                                ci.lvl = 0.95) + apatheme
plot_model29_int 
```
...
# pre_conf_plot <- ggplot() +
#  geom_point(data=ja_df4, aes(GMC_PRE_CONF, WOA)) + 
#  geom_point(data=model29_robust, aes(x=GMC_PRE_CONF, y=fit), color="blue") +
#  geom_line(data=model29_robust, aes(x=GMC_PRE_CONF, y=fit), color="blue") +
#  geom_ribbon(data= model29_robust, aes(x=GMC_PRE_CONF, ymin=lower, ymax=upper), alpha= 0.3, fill="blue") +
#  labs(x="Pre-Advice Confidence", y="WOA")

# pre_conf_plot

## The above plot does not work with an rlmer model (a robust model)
...
# Pre-confidence plot
pre_conf_plot <- plot_model(model29_robust, type = "pred", #can use the same model type as before from sjplot
                                terms = "GMC_PRE_CONF", 
                                title = " ", 
                                axis.title = c("Pre-Advice Confidence (grand-mean centered)", "Weight of Advice"), 
                                show.data = TRUE, dot.size = 0.8, jitter = 0.2, 
                                ci.lvl = 0.95) + apatheme
pre_conf_plot 
...
per_advice_acc_plot <- plot_model(model29_robust, type = "pred", #can use the same model type as before from sjplot
                                terms = "GMC_PERCEIVED_ACC", 
                                title = " ", 
                                axis.title = c("Perceived Advice Accuracy (grand-mean centered)", "Weight of Advice"), 
                                show.data = TRUE, dot.size = 0.8, jitter = 0.2, 
                                ci.lvl = 0.95) + apatheme
per_advice_acc_plot
...
## Create APA bar graph

graph1 <- ja_df4 %>%
  ggplot(aes(x= AGEGROUP, fill = EMO_REG_TYPE)) +
    geom_bar(position = "dodge") +
  scale_x_discrete(name = "Age Group", labels = c('Young','Middle-aged','Older')) +
  scale_y_discrete(name = "Count") +
  scale_fill_brewer(palette = "Greys", name = "Emotion Regulation Method", breaks = c("NONE", "ACCEPT", "CHANGE"),
                    labels=c("None", "Acceptance", "Change")) +
  theme_apa(base_size = 14)


graph1 

# When needing to change legend labels and titles, as well as change the colours,
# use the scale_fill_brewer command. Add in theme_apa at the end - Otherwise errors.
...
contingency_table1 <- table(ja_df4$AGEGROUP, ja_df4$EMO_REG_TYPE)

### contingency_table1 <- melt(contingency_table1)

contingency_table1

chisq.theo.multcomp(contingency_table1, p.method = "bonferroni") # THIS WORKS!

# Combine the age group and emotion regulation method columns

## contingency_table1 <- contingency_table1 %>% 
##  unite('AGE_EMO', Var1:Var2, remove = TRUE)

## contingency_table1
  
## pairwise.prop.test(contingency_table1) ## Will not work due to there being more than 2 columns


## chisq.posthoc.test(contingency_table1) # Works with more than 2 columns

## pairwise.prop.test(contingency_table1, c(rep(sum(contingency_table1),nlevels(contingency_table1))))

## chisq.test(contingency_table1)

## pairwiseNominalIndependence(contingency_table1,
##                            fisher = FALSE,
##                            gtest  = FALSE,
##                            chisq  = TRUE,
##                            method = "fdr")
