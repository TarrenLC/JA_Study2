...
emo_type_graph <- ja_df4 %>%
  ggplot(aes(x= AGEGROUP, fill = EMO_REG_TYPE)) +
    geom_bar(position = "dodge") +
  scale_x_discrete(name = "Age Group", labels = c('Young','Middle-aged',
                                                  'Older')) +
  scale_y_discrete(name = "Count") +
  scale_fill_brewer(palette = "Greys", name = "Emotion Regulation Method", 
...
chi_change <- ja_df4 %>% filter(EMO_CHNG_EFFORT >= 1)

chi_accept <- ja_df4 %>% filter(EMO_ACPT_EFFORT >= 1)

chi_none <- ja_df4 %>% filter(EMO_REGULATOR == "NO")
